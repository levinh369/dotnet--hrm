...
namespace DACN.Areas.Admin.Controllers
{
    ...
    public class PositionController : Controller
    {
        ...
        {
            var (entities, total) = await positionRepository.GetPagedAsync(page, pageSize, keySearch, fromDate, DateTo, isActive);
            var data = entities.Select(j => new PositionResponse
            {
                Id = j.PositionId,
        ...
        public async Task<IActionResult> Detail(int id)
        {
            ...
            var dep = await positionRepository.GetByIdAsync(id);
            if (dep == null)
                return Json(new { success = false, message = "Không tìm thấy bản ghi" });

            // Map sang DTO
            var depDto = new PositionResponse
            {
                Id = dep.PositionId,
                PositionName = dep.PositionName,
        ...
        public async Task<IActionResult> EditPost(int Id, PositionRequest dto)
        {
            ...
            try
            {
                await positionRepository.UpdatePositionAsync(Id, dto);
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Create(PositionRequest dto)
        {
            // 1. Validate ModelState
            if (!ModelState.IsValid)
            {
            ...
            try
            {
                await positionRepository.CreatePositionAsync(dto);

                return Json(new { success = true, message = "Đã tạo chức vụ thành công!" });
            }
            catch (Exception ex)
            {
                // Lấy message lỗi từ InnerException nếu có
